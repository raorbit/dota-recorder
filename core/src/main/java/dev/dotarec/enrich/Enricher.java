...
public class Enricher {
    ...
    public void enrich(long matchRowId, long dotaMatchId) {
        try {
            ...
            // NOTE: we deliberately do NOT short-circuit an unmappable-hero row before the fetch. A
            // configured account id can still attribute the row by account (a public profile whose
            // OpenDota row carries our account_id), regardless of hero mappability, so skipping the
            // fetch would wrongly withhold enrichment the account join delivers. The newer-hero HOLD is
            // applied only AFTER attribution fails (see fullEnrich), which costs one GET per retry for a
            // genuinely unattributable row (private profile + newer-than-map hero) until a map refresh.
