...
describe('applyMatchesDeleted (full delete)', () => {
  it('drops the deleted rows and their clips', () => {
    const next = applyMatchesDeleted(slice(), new Set([1]));
    expect(next.matches.map((m) => m.id)).toEqual([2, 3]);
    // Match 1's two clips cascade away server-side; the local list mirrors that.
...
describe('applyMatchVideosDeleted (keep-clips delete)', () => {
